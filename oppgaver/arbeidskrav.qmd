...
# Generate data for positive x values only
data <- data.frame(x = seq(0, 10, by = 0.1))
data$y <- cost_function(data$x)

# Optimal levels of output for positive x values
x_optimal_100 <- sqrt(50)
x_optimal_150 <- sqrt(75)
...
plot <- ggplot(data, aes(x = x, y = y)) +
  ...
  geom_vline(xintercept = x_optimal_100, linetype = "dashed", color = "green", size = 1) +
  geom_vline(xintercept = x_optimal_150, linetype = "dotted", color = "purple", size = 1) +
  ggtitle("Cost Function c(x) = 2x^2 (Positive x) with Price and Optimal Output Levels") +
  xlab("x") +
  ylab("c(x)") +
...
```{r, echo=F}
fp <- '/home/joernih/homepageJIH/nonpublic/templates/mikro/script/oppgaver_h_2023.py'
